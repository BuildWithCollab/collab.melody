// Implementation of collab.melody.render. The mixing math and the
// voice-variant dispatch via voice.match(...) live here so they
// don't burden the interface module's BMI.
...
// Mix one per-sample contribution into `buf`. Per-sample clamp keeps
// int32 overflow impossible even with extreme polyphony; we clip down
// to int16 in finalize_mix.
...
[[nodiscard]] int voice_start_ms_local(const Voice& voice) {
...
[[nodiscard]] int derive_duration_ms(const Melody& melody) {
    ...
    for (const auto& v : melody.voices) {
        const int end = voice_start_ms_local(v) + voice_duration_ms(v);
